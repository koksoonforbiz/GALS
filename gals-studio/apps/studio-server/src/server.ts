...
export async function buildServer(): Promise<FastifyInstance> {
  ...
  if (existsSync(webDist)) {
    const MIME: Record<string, string> = { '.js': 'text/javascript', '.css': 'text/css', '.html': 'text/html', '.svg': 'image/svg+xml', '.json': 'application/json' };
    app.get('/*', async (req, reply) => {
      const urlPath = (req.params as Record<string, string>)['*'] ?? '';
      const candidate = resolve(join(webDist, urlPath));
      const file = candidate.startsWith(webDist) && urlPath && existsSync(candidate) ? candidate : join(webDist, 'index.html');
