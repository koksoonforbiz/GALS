...
export async function mediaRoutes(app: FastifyInstance): Promise<void> {
  app.get<{ Params: { sessionId: string; snapshotId: string } }>(
    ...
    (req, reply) => serveMedia(req, reply, [req.params.sessionId, 'snapshots', `${req.params.snapshotId}.html`]),
  );
  app.get<{ Params: { sessionId: string; snapshotId: string } }>(
    '/api/media/snapshot/:sessionId/:snapshotId.jpg',
    (req, reply) => serveMedia(req, reply, [req.params.sessionId, 'snapshots', `${req.params.snapshotId}.jpg`]),
  );
