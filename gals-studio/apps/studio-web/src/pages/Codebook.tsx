...
export function Codebook() {
  ...
  async function lock() {
    ...
    if (
      !window.confirm(
        `Lock ${current.name} v${current.version}? Locking is required before gold coding; editing a locked version forks a new one.`,
      )
    )
      return;
        ...
        >
          {versions.map((v) => (
            <option key={v.id} value={v.id}>
              {v.name} v{v.version}{' '}
              {v.locked ? <Lock size={12} className="inline align-[-2px]" /> : ''}
            </option>
          ))}
        </select>
        {current && !current.locked && (
          <button onClick={lock} className="rounded bg-slate-900 px-3 py-1 text-sm text-white">
            Lock version
          </button>
        )}
        {current?.locked && (
          <span className="text-sm text-amber-600">Locked — editing forks a new version</span>
        )}
      </div>
              ...
              {(def.codes ?? [])
                ...
                .map((c: any) => (
                  <div key={c.key} className="flex items-center gap-2 text-sm">
                    ...
                    <span className="truncate text-xs text-slate-400" title={c.definition}>
                      {c.definition}
                    </span>
                  </div>
                ))}
