...
export function Coding() {
  ...
  const [pass, setPass] = useState<string>(() => localStorage.getItem('gals.pass') ?? 'primary_rater_1');
  ...
  useEffect(() => {
    if (activeWindow) localStorage.setItem(`gals.active.${sessionId}.${coderId}.${pass}`, String(active));
  ...
  const assign = useCallback(
    async (code: CodeDef) => {
      ...
      const base = { sessionId, windowId: activeWindow.id, coderId, codingPass: pass, dimension: code.dimension, code: code.key, codingMs };
    ...
    [activeWindow, coderId, pass, autoAdvance, state, sessionId, advanceToNextUncoded, refreshState],
  );

  const clearAffect = useCallback(async () => {
    if (!activeWindow || !coderId) return;
    await api.clearAnnotation({ sessionId, windowId: activeWindow.id, coderId, codingPass: pass, dimension: 'affect' });
    void refreshState();
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      if (e.key === ' ') { e.preventDefault(); ph.playing ? store.pause() : playWindow(); return; }
      if (e.key === '[' || e.key === 'ArrowLeft') { gotoWindow(active - 1); return; }
      if (e.key === ']' || e.key === 'ArrowRight') { gotoWindow(active + 1); return; }
      if (e.key === 'n') { e.preventDefault(); notesRef.current?.focus(); return; }
      if (e.key === 'u') { const u = codebook.codes.find((c) => c.key === 'unclear'); if (u) void assign(u); return; }
      if (e.key === '0') { void clearAffect(); return; }
      const code = shortcutMap.get(e.key);
      if (code) { e.preventDefault(); void assign(code); }
    };
  ...
  if (!meta || !state) {
    return (
      <div className="p-8 text-center text-slate-400">
        ...
        {meta && coders.length === 0 && <div className="mt-3"><CreateCoder onCreated={(c) => { setCoders([c]); setCoderId(c.id); }} /></div>}
      </div>
    );
  }

  const disagreementWinIds = new Set(disagreements.filter((d) => !d.resolved).map((d) => d.windowId));
  const visibleWindows = windows.filter((w) => {
    if (filter === 'uncoded') return !state.annotations[w.id]?.affect || !state.annotations[w.id]?.behavior;
  ...
  return (
    <div className="space-y-2">
      <div className="flex flex-wrap items-center gap-2 rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm">
        <Link to="/" className="text-slate-400 hover:text-slate-700">← Library</Link>
        <span className="font-semibold">{meta.session.userDisplayName ?? meta.session.userId.slice(0, 8)}</span>
        <span className="text-slate-400">|</span>
        <label>I am:</label>
        <select value={coderId} onChange={(e) => setCoderId(e.target.value)} className="rounded border border-slate-300 px-2 py-1">
          <option value="">— select coder —</option>
          {coders.map((c) => <option key={c.id} value={c.id}>{c.name} ({c.role})</option>)}
        </select>
        <CreateCoder onCreated={(c) => { setCoders((cs) => [...cs, c]); setCoderId(c.id); }} />
        <label>Pass:</label>
        <select value={pass} onChange={(e) => setPass(e.target.value)} className="rounded border border-slate-300 px-2 py-1">
          {PASSES.map((p) => <option key={p} value={p}>{p}</option>)}
        </select>
        <span className="ml-auto text-slate-500">
          {state.progress.codedAffect}/{state.progress.total} affect · {state.progress.missingAffect} missing
      ...
      <div className="grid grid-cols-[200px_1fr_280px] gap-2">
        ...
        <div className="max-h-[80vh] overflow-auto rounded-lg border border-slate-200 bg-white">
          <div className="sticky top-0 flex gap-1 border-b border-slate-200 bg-white p-2 text-[11px]">
            {(['all', 'uncoded', 'disagreements'] as const).map((f) => (
              <button key={f} onClick={() => setFilter(f)} className={`rounded px-1.5 py-0.5 ${filter === f ? 'bg-slate-900 text-white' : 'bg-slate-100'}`}>{f}</button>
            ))}
          </div>
          {visibleWindows.map((w) => {
            ...
            return (
              <button key={w.id} onClick={() => gotoWindow(idx)} className={`flex w-full items-center gap-1 border-b border-slate-100 px-2 py-1 text-left text-xs ${idx === active ? 'bg-sky-50 ring-1 ring-inset ring-sky-400' : 'hover:bg-slate-50'}`}>
                <span className="w-6 text-slate-400">{w.index}</span>
                <span className="w-12 font-mono text-[10px] text-slate-400">{fmtRel(w.startWallMs - meta.baseWallClockMs)}</span>
                {a?.affect && <span className="h-3 w-3 rounded" style={{ backgroundColor: colorOf(a.affect.code) }} title={a.affect.code} />}
        ...
        <div className="space-y-2">
          <div className="grid grid-cols-[1fr_240px] gap-2">
            <DomStage sessionId={sessionId} snapshot={currentSnapshot} gaze={null} lastClick={null} aoiVisible={{}} showScreenshot={false} />
            <WebcamPanel segments={(sparse?.webcam ?? []) as WebcamSeg[]} absoluteMs={absoluteMs} playing={ph.playing} />
          </div>

          {activeWindow && <WindowScrubber win={activeWindow} base={meta.baseWallClockMs} offsetMs={ph.offsetMs} onSeek={(o) => store.seek(o)} />}

          <div className="flex items-center gap-2 rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm">
            <button onClick={() => gotoWindow(active - 1)} className="rounded border border-slate-300 px-2 py-1">[ Prev</button>
            <button onClick={playWindow} className="rounded bg-slate-900 px-3 py-1 text-white">▶ Play window</button>
            <button onClick={() => store.pause()} className="rounded border border-slate-300 px-2 py-1">⏸</button>
            <button onClick={() => gotoWindow(active + 1)} className="rounded border border-slate-300 px-2 py-1">Next ]</button>
            {activeWindow && <span className="font-mono text-xs text-slate-400">window {activeWindow.index} · {fmtRel(activeWindow.startWallMs - meta.baseWallClockMs)}–{fmtRel(activeWindow.endWallMs - meta.baseWallClockMs)}</span>}
            <span className="ml-auto text-emerald-600">{winAnns?.affect && winAnns?.behavior ? 'saved ✓' : 'needs affect+behavior'}</span>
          </div>
        ...
        <div className="max-h-[80vh] overflow-auto rounded-lg border border-slate-200 bg-white p-2">
          {(['affect', 'behavior', 'ef_event', 'motivation'] as Dimension[]).map((dim) => (
            <div key={dim} className="mb-3">
              <div className="mb-1 text-[11px] font-semibold uppercase tracking-wide text-slate-400">{dim}</div>
              <div className="flex flex-wrap gap-1">
                {codebook.codes.filter((c) => c.dimension === dim).map((c) => {
                  const selected =
                    (dim === 'affect' && winAnns?.affect?.code === c.key) ||
                    (dim === 'behavior' && winAnns?.behavior?.code === c.key) ||
                    ((dim === 'ef_event' || dim === 'motivation') && (winAnns?.[dim] as any[])?.some((a) => a.code === c.key));
                  return (
                    <button key={c.key} onClick={() => assign(c)} title={`${c.definition}\n${c.inclusion ?? ''}`}
                      className="flex items-center gap-1 rounded px-1.5 py-0.5 text-xs"
                      style={{ backgroundColor: c.color + (selected ? '44' : '22'), color: c.color, boxShadow: selected ? `0 0 0 2px ${c.color}` : undefined }}>
                      <kbd className="rounded bg-white/70 px-1 font-mono text-[10px]">{c.shortcut}</kbd>
                      {c.label}
                    </button>
                  );
                })}
          ...
          <textarea ref={notesRef} placeholder="Notes (n)…" className="mt-2 w-full rounded border border-slate-300 p-2 text-xs" rows={3}
            onBlur={(e) => {
              if (activeWindow && coderId && winAnns?.affect)
                void api.upsertAnnotation({ sessionId, windowId: activeWindow.id, coderId, codingPass: pass, dimension: 'affect', code: winAnns.affect.code, notes: e.target.value });
            }} />
          {pass === 'tiebreaker' && <button onClick={async () => { await api.deriveGold(sessionId); void refreshState(); }} className="mt-2 w-full rounded bg-emerald-600 px-2 py-1 text-sm text-white">Derive gold consensus</button>}
        </div>
      </div>
...
function WindowScrubber({ win, base, offsetMs, onSeek }: { win: any; base: number; offsetMs: number; onSeek: (o: number) => void }) {
  ...
  return (
    <div className="relative h-8 cursor-pointer rounded bg-slate-100"
      onClick={(e) => {
        const rect = e.currentTarget.getBoundingClientRect();
        onSeek(viewStart + ((e.clientX - rect.left) / rect.width) * span);
      }}>
      <div className="absolute top-0 h-full bg-sky-200/70" style={{ left: `${pct(startOff)}%`, width: `${pct(endOff) - pct(startOff)}%` }} />
      <div className="absolute top-0 h-full w-0.5 bg-slate-900" style={{ left: `${pct(offsetMs)}%` }} />
      <span className="absolute left-1 top-1 text-[10px] text-slate-400">±5s context · 20s window shaded</span>
    </div>
  );
}

function DisagreementCompare({ disagreements, activeWindowId }: { disagreements: any[]; activeWindowId?: string }) {
...
function CreateCoder({ onCreated }: { onCreated: (c: any) => void }) {
  ...
  if (!open) return <button onClick={() => setOpen(true)} className="rounded border border-slate-300 px-2 py-1 text-xs">+ coder</button>;
  return (
    <span className="flex items-center gap-1">
      <input value={name} onChange={(e) => setName(e.target.value)} placeholder="name" className="rounded border border-slate-300 px-1 py-0.5 text-xs" />
      <select value={role} onChange={(e) => setRole(e.target.value)} className="rounded border border-slate-300 px-1 py-0.5 text-xs">
        {['trained_rater', 'tiebreaker', 'teacher', 'expert'].map((r) => <option key={r}>{r}</option>)}
      </select>
      <button onClick={async () => { if (!name) return; const c = await api.createCoder(name, role); onCreated(c); setOpen(false); setName(''); }} className="rounded bg-slate-900 px-2 py-0.5 text-xs text-white">add</button>
    </span>
  );
}
...
function HelpOverlay({ onClose }: { onClose: () => void }) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={onClose}>
      <div className="w-[480px] rounded-lg bg-white p-5 text-sm" onClick={(e) => e.stopPropagation()}>
        <h2 className="mb-3 text-lg font-bold">Keyboard shortcuts</h2>
        <ul className="space-y-1">
          <li><kbd>space</kbd> play/pause window · <kbd>[</kbd> <kbd>]</kbd> / arrows: prev/next window</li>
          <li><kbd>1</kbd>–<kbd>9</kbd> affect · <kbd>q w e r t y u</kbd> behavior</li>
          <li><kbd>a s d f g h j k</kbd> EF events · <kbd>z x c v</kbd> motivation</li>
          <li><kbd>u</kbd> unclear · <kbd>0</kbd> clear affect · <kbd>n</kbd> notes · <kbd>?</kbd> this help</li>
        </ul>
        <button onClick={onClose} className="mt-4 rounded bg-slate-900 px-3 py-1 text-white">Close</button>
      </div>
    </div>
  );
}
