          ...
          >
            {sessions.map((s) => (
              <option key={s.id} value={s.id}>
                {s.userDisplayName ?? s.id.slice(0, 8)}
              </option>
            ))}
          </select>
        )}
...
function Reliability({ sessions }: { sessions: any[] }) {
  ...
  const kappaTone = data.cautionHighKappa
  ...
  return (
    <div className="space-y-3">
      <div className="flex gap-2 text-sm">
        ...
        <button
          onClick={async () => {
            await api.saveReliability({ scope, dimension, save: true });
            setSaved(true);
      ...
      <div className="grid grid-cols-2 gap-3 md:grid-cols-3 lg:grid-cols-6">
        <Card
          label="Cohen's κ"
          value={fmt(data.kappa)}
          tone={kappaTone}
          sub={
            data.cautionHighKappa ? (
              <span className="inline-flex items-center gap-1">
                <TriangleAlert size={12} /> &gt;0.85 caution
              </span>
            ) : data.kappa >= 0.6 ? (
              'substantial'
            ) : (
        ...
        <Card
          label="PABAK"
          value={fmt(data.pabak)}
          sub={gap > 0.2 ? `+${fmt(gap)} vs κ — prevalence-skewed` : 'close to κ'}
        />
      ...
      {data.cautionHighKappa && (
        <div className="rounded border border-amber-400 bg-amber-50 p-2 text-sm text-amber-800">
          κ &gt; 0.85 — treat with skepticism (likely collapsed categories). Always read PABAK
          alongside κ.
        </div>
      )}
      ...
      <div className="rounded-lg border border-slate-200 bg-white p-3">
        <h3 className="mb-2 text-sm font-semibold text-slate-500">
          Per-code agreement (which codes drag reliability down)
        </h3>
        <table className="w-full text-sm">
          ...
          <tbody>
            {Object.entries(data.perCode as Record<string, any>)
              .sort((a, b) => a[1].agreement - b[1].agreement)
              .map(([code, c]) => (
                <tr key={code} className="border-t border-slate-100">
                  <td>{code}</td>
                  <td>{Math.round(c.agreement * 100)}%</td>
                  <td>{c.total}</td>
                </tr>
              ))}
          </tbody>
...
function ConfusionMatrix({ cm }: { cm: { labels: string[]; matrix: number[][] } }) {
  ...
  return (
    <div className="overflow-auto rounded-lg border border-slate-200 bg-white p-3">
      <h3 className="mb-2 text-sm font-semibold text-slate-500">
        Confusion matrix (rater 1 rows × rater 2 cols)
      </h3>
      <table className="text-xs">
        <thead>
          <tr>
            <th></th>
            {cm.labels.map((l) => (
              <th key={l} className="px-1 text-slate-400">
                {l.slice(0, 6)}
              </th>
            ))}
          </tr>
            ...
            <tr key={i}>
              ...
              {row.map((v, j) => (
                <td
                  ...
                  style={{
                    backgroundColor: `rgba(14,165,233,${v / max})`,
                    color: v / max > 0.5 ? 'white' : '#0f172a',
                  }}
                >
                  {v || ''}
                </td>
...
function Dynamics({ session }: { session: string }) {
  ...
  useEffect(() => {
    api.dynamics(session, { resolutionWindows: res }).then(setData);
  }, [session, res]);
  if (!data) return <div className="text-slate-400">Loading…</div>;
  ...
  return (
    <div className="space-y-3">
      <div className="rounded-lg border border-slate-200 bg-white p-3">
        <h3 className="mb-2 text-sm font-semibold text-slate-500">
          Gold affect track ({data.track.filter((t: any) => t).length}/{data.track.length} windows)
        </h3>
        <div className="flex h-6 w-full overflow-hidden rounded">
          {data.track.map((c: string | null, i: number) => (
            <div
              key={i}
              className="flex-1"
              style={{ backgroundColor: c ? colorOf(c) : '#e2e8f0' }}
              title={`#${i}: ${c ?? 'uncoded'}`}
            />
          ))}
        </div>
      ...
      <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
        <div className="rounded-lg border border-slate-200 bg-white p-3">
          ...
          {Object.entries(data.prevalence.counts as Record<string, number>)
            .sort((a, b) => b[1] - a[1])
            .map(([k, v]) => (
              <div key={k} className="flex items-center gap-2 text-xs">
                ...
                <div className="h-3 flex-1 rounded bg-slate-100">
                  <div
                    ...
                    style={{
                      width: `${(data.prevalence.proportions[k] * 100).toFixed(0)}%`,
                      backgroundColor: colorOf(k),
                    }}
                  />
                </div>
                <span className="w-16 text-right text-slate-400">
                  {v} ({Math.round(data.prevalence.proportions[k] * 100)}%)
                </span>
              </div>
            ))}
        </div>
      ...
      <div className="rounded-lg border border-slate-200 bg-white p-3">
        <div className="mb-2 flex items-center gap-2">
          ...
          <label className="ml-auto text-xs">
            ...
            <input
              ...
              className="ml-1 w-12 rounded border border-slate-300 px-1"
            />
          </label>
        </div>
        {data.unresolved.length === 0 ? (
          <div className="text-sm text-emerald-600">
            ...
            <ArrowRight size={12} className="inline align-[-2px]" />
            frustration
            <ArrowRight size={12} className="inline align-[-2px]" />
            boredom cascades.
          </div>
          ...
          data.unresolved.map((e: any, i: number) => (
            ...
            >
              <TriangleAlert size={12} className="inline align-[-2px]" /> windows {e.startWindow}–
              {e.endWindow}:{' '}
              {e.sequence.map((s: string, j: number) => (
                <span key={j}>
                  {j > 0 && <ArrowRight size={12} className="mx-0.5 inline align-[-2px]" />}
                  {s}
                </span>
              ))}
            </div>
        ...
        <div className="mt-1 text-[11px] text-slate-400">
          Resolved (productive) confusion is excluded — only escalating, unresolved episodes are
          flagged.
        </div>
      </div>
      ...
      <table className="text-xs">
        <thead>
          <tr>
            ...
            {t.states.map((s: string) => (
              <th key={s} className="px-1 text-slate-400">
                {s.slice(0, 6)}
              </th>
            ))}
          </tr>
        ...
        <tbody>
          {t.states.map((from: string) => (
            <tr key={from}>
              <td className="pr-2" style={{ color: colorOf(from) }}>
                {from.slice(0, 10)}
              </td>
              {t.states.map((to: string) => {
                ...
                return (
                  <td
                    ...
                    style={{
                      backgroundColor: `rgba(139,92,246,${p})`,
                      color: p > 0.5 ? 'white' : '#0f172a',
                    }}
                  >
                    {p ? p.toFixed(2) : ''}
                  </td>
  ...
  return (
    <div className="space-y-3">
      <div className="grid grid-cols-2 gap-3">
        <Card
          label="Session allocation score"
          value={fmt(data.sessionAllocation)}
          sub="TV distance from expected (0=match, 1=disjoint)"
        />
        <div className="rounded-lg border border-slate-200 bg-white p-3">
          <h3 className="mb-1 text-sm font-semibold text-slate-500">Session PDT</h3>
          {Object.entries(data.pdt as Record<string, number>)
            .sort((a, b) => b[1] - a[1])
            .map(([r, v]) => (
      ...
      <div className="rounded-lg border border-slate-200 bg-white p-3">
        <h3 className="mb-2 text-sm font-semibold text-slate-500">
          Epochs ({data.epochs.length}) · allocation_score per epoch
        </h3>
        <table className="w-full text-xs">
          ...
          <tbody>
            {data.epochs.map((e: any, i: number) => {
              const dom = Object.entries(e.pdt as Record<string, number>).sort(
                (a, b) => b[1] - a[1],
              )[0];
              return (
                <tr key={i} className="border-t border-slate-100">
                  <td>{e.type}</td>
                  <td>{fmt(e.allocationScore)}</td>
                  <td>{dom ? `${dom[0]} ${Math.round(dom[1] * 100)}%` : '—'}</td>
                </tr>
              );
            })}
          </tbody>
      ...
      <div className="rounded-lg border border-slate-200 bg-white p-3">
        <h3 className="mb-2 text-sm font-semibold text-slate-500">
          Gaze on-screen by quartile (sustained-attention proxy)
        </h3>
        <div className="flex gap-2">
          {data.quartiles.map((q: number, i: number) => (
            <div key={i} className="flex-1 text-center text-xs">
              <div className="text-slate-400">Q{i + 1}</div>
              <div className="text-lg font-bold">{Math.round(q * 100)}%</div>
            </div>
          ))}
        </div>
      </div>
...
function Reading({ session }: { session: string }) {
  const [data, setData] = useState<any>(null);
  useEffect(() => {
    api.reading(session).then(setData);
  }, [session]);
  if (!data) return <div className="text-slate-400">Loading…</div>;
  return (
    <div className="space-y-3">
      {data.windowScrollOnly && (
        <div className="rounded border border-amber-400 bg-amber-50 p-2 text-sm text-amber-800">
          Low-confidence: only window scroll available (older bundle). Window scrollY is unreliable
      ...
      <div className="rounded-lg border border-slate-200 bg-white p-3">
        ...
        <div className="mt-2 text-[11px] text-slate-400">
          Derived from scrollHosts + PDF pages. Window scrollY is never used for progress.
        </div>
      </div>
    </div>
  );
...
function GroundTruth({ session }: { session: string }) {
  ...
  return (
    <div className="space-y-3">
      <div className="rounded-lg border border-slate-200 bg-white p-3">
        <h3 className="mb-2 text-sm font-semibold text-slate-500">
          ESM trajectory ({data.esmTrajectory.length} probes)
        </h3>
      ...
      <div className="rounded-lg border border-slate-200 bg-white p-3">
        <h3 className="mb-2 text-sm font-semibold text-slate-500">
          Convergent validity (across all sessions; expected r ≈ 0.30–0.50)
        </h3>
        <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
          <Scatter
            title={
              <>
                AEQ-S boredom <ArrowLeftRight size={12} className="inline align-[-2px]" /> coded
                boredom
              </>
            }
            v={cv.aeqBoredom_vs_codedBoredom}
          />
          <Scatter
            title={
              <>
                PANAS NA <ArrowLeftRight size={12} className="inline align-[-2px]" /> coded
                frustration
              </>
            }
            v={cv.panasNA_vs_codedFrustration}
          />
          <Scatter
            title={
              <>
                IMI interest <ArrowLeftRight size={12} className="inline align-[-2px]" /> ESM
                engagement
              </>
            }
            v={cv.imiInterest_vs_esmEngagement}
          />
        </div>
      </div>
          ...
          data.questionnaires.map((q: any, i: number) => (
            <div key={i} className="text-xs">
              <span className="font-semibold">
                {q.instrument} ({q.phase}):
              </span>{' '}
              {JSON.stringify(q.scoredSubscales)}
            </div>
          ))
        )}
      </div>
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} className="w-full" style={{ height: 140 }}>
      {keys.map((k) => {
        ...
        const d = pts.length
          ? 'M' + pts.map((s) => `${x(s.t).toFixed(1)},${y(s[k]).toFixed(1)}`).join(' L')
          : '';
        return <path key={k} d={d} fill="none" stroke={colors[k]} strokeWidth={1.5} />;
      })}
      {keys.map((k, i) => (
        <text key={k} x={4} y={12 + i * 12} fontSize={9} fill={colors[k]}>
          {k}
        </text>
      ))}
    </svg>
...
function Scatter({
  ...
  title: ReactNode;
  v: { points: { x: number; y: number }[]; r: number };
}) {
  const W = 180,
    H = 140;
  const xs = v.points.map((p) => p.x),
    ys = v.points.map((p) => p.y);
  ...
  return (
    <div>
      ...
      <div className="text-xs text-slate-400">
        r = {Number.isFinite(v.r) ? v.r.toFixed(2) : 'n/a'} · n = {v.points.length}
      </div>
      <svg
        viewBox={`0 0 ${W} ${H}`}
      ...
      >
        {v.points.map((p, i) => (
          <circle key={i} cx={px(p.x)} cy={py(p.y)} r={3} fill="#8b5cf6" />
        ))}
      </svg>
    </div>
  );
...
function Export({ sessions }: { sessions: any[] }) {
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="text-xs text-slate-400">
        Headless equivalent: <code>studio-analyze --scope {scope} --out ./analysis-out</code>
      </div>
    </div>
  );
}
