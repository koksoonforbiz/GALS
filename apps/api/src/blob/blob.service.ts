...
export class BlobService implements OnModuleInit {
  ...
  constructor(private readonly config: ConfigService) {
    ...
    // We return presigned URLs as relative `/s3/...` paths that the web dev server
    // proxies to MinIO. The upstream Host seen by MinIO is therefore the internal
    // endpoint host (e.g. `minio:9000` in docker), so signatures must be generated
    // against the internal endpoint to avoid signature/host mismatches.
