...
export class PyfeatService {
  ...
  async retryJob(jobId: string): Promise<PyfeatJob | null> {
    const job = await this.prisma.pyfeatJob.findUnique({ where: { id: jobId } });
    if (!job || job.status !== 'FAILED') return null;

    const [segment, session, config] = await Promise.all([
      this.prisma.recordingSegment.findFirst({
        where: { pyfeatJobId: job.id },
        select: { startWallTime: true },
      }),
      this.prisma.studentSession.findUnique({
        where: { id: job.sessionId },
        select: { startedAt: true },
      }),
      this.getConfig(job.courseId),
    ]);

    const clipStartWallTime = (segment?.startWallTime ?? session?.startedAt ?? new Date()).toISOString();

    await this.prisma.pyfeatJob.update({
      where: { id: job.id },
      data: {
        status: 'PENDING',
        error: null,
    ...
    const payload = JSON.stringify({
      ...
      sourceMinioKey: job.sourceMinioKey,
      extractionFps: config.extractionFps,
      enabledAus: config.enabledAus,
      detectorBackend: config.detectorBackend,
      auPredictor: config.auPredictor,
    ...
    await this.redis.rpush(PYFEAT_QUEUE_KEY, payload);
    this.logger.log(`Retried py-feat job ${job.id} for session ${job.sessionId}`);

    return this.prisma.pyfeatJob.findUnique({ where: { id: job.id } });
  }
