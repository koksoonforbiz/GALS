...
1. Root `.env` from `.env.example` — every `:?required` value in `docker-compose.yml` (Postgres creds, MinIO root creds, `JWT_SECRET`, SMTP, Bedrock token/region). Before real users also set: `LLM_DAILY_COST_CAP_USD` (checklist item 16), `ENCRYPTION_KEY` (item 4 — before the first secret is ever encrypted), `MFA_REQUIRED_ROLES=admin,teacher` (item 12 — staff must enrol 2FA on first sign-in; warn them), and `BEDROCK_GUARDRAIL_ID`/`BEDROCK_GUARDRAIL_VERSION` once the Guardrail exists in the AWS account (items 62/63). Do not set `ALLOWED_ORIGINS` here — the door file overrides it.
